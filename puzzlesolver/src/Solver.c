...
int solve(Entry** entries, PyObject* puzzle) {
    ...
    for (int i = 0; i < PyList_Size(solutions); i++) {
        ...
        Py_XINCREF(puzzle);
        if (setRemotenessPyObject(entries, puzzle, 0) < 0)
            goto err;
    ...
    while (stack != NULL) {
        ...
        PyTuple_SetItem(args, 0, PyUnicode_FromString("undo"));

        if ((moves = PyObject_CallMethod(puzzle, "generateMoves", "O", args)) == NULL) 
            goto err;
